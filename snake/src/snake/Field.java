...
public class Field {
	...
	public Location getFreeLocation() {
		Location location = new Location(rand.nextInt(depth), rand.nextInt(width));
		while (getObjectAt(location) != null) {
			location = new Location(rand.nextInt(depth), rand.nextInt(width));
		}
		return location;
	}
